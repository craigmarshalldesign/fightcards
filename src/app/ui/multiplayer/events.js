...
export function normalizeLobbyRecord(raw) {
  if (!raw) return null;
  const lobby = { ...raw };
...
async function createLobby() {
  ...
  try {
    ...
    const success = await runTransactions(db.tx.lobbies[lobbyId].set(lobby), {
      onError(error) {
...
async function claimSeat(seat) {
  ...
  if (lobby[seatUserKey]) {
    if (lobby[seatUserKey] === user.id) return;
    return;
  }

  const opponentSeatKey = seat === 'host' ? 'guestUserId' : 'hostUserId';
  if (lobby[opponentSeatKey] === user.id) {
    return;
  }
  ...
  const displayNameKey = seat === 'host' ? 'hostDisplayName' : 'guestDisplayName';
  const displayName = deriveDisplayName(user);
  if (displayName) {
    updates[displayNameKey] = displayName;
  }
...
async function leaveSeat(seat) {
  ...
  const displayNameKey = seat === 'host' ? 'hostDisplayName' : 'guestDisplayName';
  const colorKey = seat === 'host' ? 'hostColor' : 'guestColor';
  const readyKey = seat === 'host' ? 'hostReady' : 'guestReady';
  updates[displayNameKey] = '';
  updates[colorKey] = '';
  updates[readyKey] = false;
...
async function startMatch() {
  ...
  try {
    ...
    const success = await runTransactions(
      [
        db.tx.matches[matchId].set(match),
        db.tx.matchEvents[eventId].set(matchStartedEvent),
    ...
    seedMultiplayerMatch(matchId, lobby.hostUserId, lobby.guestUserId, lobby.hostColor, lobby.guestColor, diceRolls);
    requestRender();
  } catch (error) {
    console.error('Failed to start match', error);
    state.multiplayer.lobbyList.error = 'Could not start the match. Please try again.';
...
export async function logMatchEvent(event) {
  if (!event) return;
  const { matchId, sequence } = event;
  if (!matchId || typeof sequence !== 'number') return;
  const matchActive = isMultiplayerMatchActive(matchId);
  if (!matchActive) return;
  addMultiplayerLogEvent(event);
}
